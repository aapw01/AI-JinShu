...
def node_prewrite(state: GenerationState) -> GenerationState:
    ...
    prewrite = state["prewrite_agent"].run(
        state["novel_info"],
        state["num_chapters"],
...
def node_outline(state: GenerationState) -> GenerationState:
    ...
    # Only generate first volume outlines; subsequent volumes are generated by node_volume_replan
